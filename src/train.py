...
class DQNAgent:
    ...
    def train(self, max_episode = 200, resume = False):
        ## CONFIGURE NETWORK
        ...
        if resume == False:
            ...
            device = device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            self.target_model = deepcopy(self.model).to(device)
            print("Resuming training from model ", self.model_name_prefix)
        ...
        while episode < max_episode:
            ...
            if done or trunc:
                ...
                if validation_score > self.best_val:
                   print("better model")
                   self.path = ""
                   self.save(self.path)
                   self.best_val = validation_score
